...
def Comparison_RowWidthDistribution(
    dataFiles: list[dataAnalysis], pathToOutput, layer=4, name="", minTimes=None, maxTimes=None, minTime=400000, maxTime=600000, excludeCrossTalk=True
):
...
def Comparison_AngleDistribution(
    ...
    plot.set_config(axs, ylim=(0, None), xlim=xlim, title="Angle Distribution", legend=True, xlabel="Equivalent Angle [Degrees]", ylabel="Frequency")
...
def fit_dataFile(
    dataFile: dataAnalysis, depth: depthAnalysis, depthCorrection=True, hideLowWidths=True, fitting="histogram", measuredAttribute="Hit_Voltage"
):
    d = depth.find_d_value(dataFile)
    allXValues = []
    allYValues = []
...
def Comparison_CCE_Vs_Depth(
    ...
    for dataFile in dataFiles:
        popt, pcov, x, y, yerr = fit_dataFile(
            dataFile, depth, depthCorrection=depthCorrection, hideLowWidths=hideLowWidths, fitting=fitting, measuredAttribute=measuredAttribute
        )
...
def Scatter_Epi_Thickness_Vs_Bias_Voltage(
    ...
    for dataFile in dataFiles:
        ...
        axs.errorbar(
            dataFile.get_voltage(), t_epi, yerr=t_epi_e, fmt="none", color=cmap((1 / 48 * dataFile.get_voltage())), elinewidth=0.5, capsize=1
        )
