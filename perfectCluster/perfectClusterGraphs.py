...
for dataFile in dataFiles:
    ...
    for cluster in tqdm(
        ...
        expectedRows = np.linspace(
            ...
            np.sort(rows)[-1 * cluster.flipped] + maxWidth + maxWidth * 2 * -1 * cluster.flipped,
            maxWidth + 1,
        ).astype(int)[:-1]
        ...
        if len(missing[missing < maxWidth]) != 0:
            ...
            missingRowsList.extend(missingRow[missing < maxWidth])
            relativeRowsList.extend(missing[missing < maxWidth])
    ...
    error = np.array(
        [
            abs(rowPercent-wilsonError(EfficiencyFrequency, rowFrequency, z=-1)),
            abs(rowPercent-wilsonError(EfficiencyFrequency, rowFrequency, z=1)),
        ]
    )
    plot = plotClass(f"{config["pathToOutput"]}ClusterTracks/{dataFile.fileName}/TimeStamps/")
