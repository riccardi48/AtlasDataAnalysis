...
for dataFile in dataFiles:
    ...
    height, x = np.histogram(PRS[indexes[maxRows<128]], bins=bins, range=_range)
    plotHist(dataFile,config["pathToOutput"] + "PixelResponseSlope/byRow/",height,x,_range,xlim,name="A")
    ...
    plotHistRemoved(dataFile,config["pathToOutput"] + "PixelResponseSlope/byRow/removed/",height,x,xlim,name="A")
    height, x = np.histogram(PRS[indexes[(minRows>128)&(maxRows<248)]], bins=bins, range=_range)
    plotHist(dataFile,config["pathToOutput"] + "PixelResponseSlope/byRow/",height,x,_range,xlim,name="B")
    binCentres = (x[:-1] + x[1:]) / 2
    popt, pcov = curve_fit(
            sumOfCauchyFunc,
            binCentres[binCentres<0],
    ...
    height = height - sumOfCauchyFunc(binCentres, *popt)
    height[binCentres==0] = 0
    plotHistRemoved(dataFile,config["pathToOutput"] + "PixelResponseSlope/byRow/removed/",height,x,xlim,name="B")
    height, x = np.histogram(PRS[indexes[(minRows>248)]], bins=bins, range=_range)
    plotHist(dataFile,config["pathToOutput"] + "PixelResponseSlope/byRow/",height,x,_range,xlim,name="C")
    binCentres = (x[:-1] + x[1:]) / 2
    popt, pcov = curve_fit(
            sumOfCauchyFunc,
            binCentres[binCentres<0],
    ...
    height = height - sumOfCauchyFunc(binCentres, *popt)
    height[binCentres==0] = 0
    plotHistRemoved(dataFile,config["pathToOutput"] + "PixelResponseSlope/byRow/removed/",height,x,xlim,name="C")