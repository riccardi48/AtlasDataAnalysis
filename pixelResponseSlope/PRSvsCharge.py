...
config["filterDict"] = {"telescope":"kit","fileName":"long_term_6Gev_kit_2"}
...
for i, dataFile in enumerate(dataFiles):
    ...
    height, x = np.histogram(clusterCharges, bins=250, range=(0, 5))
...
for dataFile in dataFiles:
    ...
    array, yedges, xedges = np.histogram2d(clusterRowWidths,PRS,bins=(40,401),range=((0.5,40.5),_range))
    plot = plotClass(config["pathToOutput"] + "PixelResponseSlope/ClusterCharge/")
    axs = plot.axs
    axs.imshow(array,aspect='auto',origin="lower",extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]])
    plot.set_config(
    ...
    plot.saveToPDF(f"PRS_RowWidth_Scatter_{dataFile.fileName}")
    
    array, yedges, xedges = np.histogram2d(clusterCharges,clusterRowWidths,bins=(40,40),range=((0,20),(0.5,40.5)))
    plot = plotClass(config["pathToOutput"] + "PixelResponseSlope/ClusterCharge/")
    axs = plot.axs
    axs.imshow(array,aspect='auto',origin="lower",extent=[xedges[0],xedges[-1],yedges[0],yedges[-1]],norm="log")
    plot.set_config(
        axs,
        title=f"Cluster Charge vs Cluster Row Width {dataFile.fileName}",
        ylabel="Cluster Charge",
        xlabel="Cluster Row Width",
        )
    plot.saveToPDF(f"Charge_RowWidth_Scatter_{dataFile.fileName}")
